...
DATA_DIR = BASE_DIR / "data"; HIST_DIR = DATA_DIR / "historical"; HIST_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="Prop Finder", version="1.0.0")
...
def remove_vig_nway(prob_map: dict):
    ...
    if s <= 0: 
        n = max(1, len(prob_map)); return {k: 1.0/n for k in prob_map}
    return {k: float(v or 0.0)/s for k,v in prob_map.items()}

def ev_decimal(d, p): d=float(d or 0.0); p=float(p or 0.0); return p*(d-1.0)-(1.0-p)
...
def compute_ev_row(r: dict):
    ...
    if market_class == "THREE_WAY" or side in ["HOME","DRAW","AWAY"]:
        ...
        return ev_decimal(price, {"HOME":ph,"DRAW":pd,"AWAY":pa}.get(side,0.0)), {"HOME":ph,"DRAW":pd,"AWAY":pa}.get(side,0.0)
...
def run_backtest(data_dir: Path, edge_threshold: float = 0.02, kelly_fraction: float = 0.5):
    ...
    kelly_star = (p * d - 1.0) / (d - 1.0); kelly_star = kelly_star.clip(0.0, 0.5)
    sel["stake"] = kelly_star * kelly_fraction

    won = sel["outcome"].astype(int)
    sel["pnl"] = np.where(won == 1, sel["stake"] * (d - 1.0), -sel["stake"])

    total_staked = sel["stake"].sum(); total_pnl = sel["pnl"].sum()
    roi = (total_pnl / total_staked) if total_staked > 0 else 0.0; hit_rate = won.mean()

    by_sport = (sel.groupby("sport").agg(n_bets=("sport","size"), pnl=("pnl","sum"), staked=("stake","sum"))
    ...
    _CACHE["summary"] = {"n_bets": int(sel.shape[0]), "roi": float(roi), "pnl": float(total_pnl),
...
def home():
    return HTMLResponse("""<!doctype html><html lang='sv'><meta charset='utf-8'>
<title>Prop Finder</title><meta name=viewport content='width=device-width,initial-scale=1'>
<link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/water.css@2/out/water.css'>
<h1>Prop Finder</h1><p>Ladda upp historik (CSV) och kör backtest.</p>
<h3>Ladda upp CSV</h3><input id=f type=file accept=.csv><button id=u>Ladda upp</button><div id=us></div>
<h3>Kör backtest</h3>
...
<button id=r>Kör</button><div id=stats></div>
<h3>Edges</h3><label>Sportfilter <input id=s placeholder='FOOTBALL'></label>
<label>Min edge <input id=me type=number step=0.01 value=0.02></label>
<button id=ref>Uppdatera</button><table id=t></table>
<script>
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    try: pd.read_csv(out, nrows=2)
    except Exception as e:
        out.unlink(missing_ok=True); return {"ok": False, "error": str(e)}
...
async def api_backtest_run(edge_threshold: float = 0.02, kelly_fraction: float = 0.5):
    return BacktestResponse(**run_backtest(DATA_DIR, edge_threshold=edge_threshold, kelly_fraction=kelly_fraction))

@app.get("/api/edges")
async def api_edges(min_edge: float = 0.02, sport: Optional[str] = None, limit: int = 500):
    res = latest_results()
    if not res or res.get("edges") is None: return []
    df = res["edges"].copy()
    if sport: df = df[df["sport"].astype(str).str.upper() == sport.upper()]
